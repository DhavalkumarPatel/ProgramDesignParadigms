...
;; screensaver-5.rkt: moving and draggable ranctangles.  
...
;; User can down and up the pen using "d" and "u" keys, When the pen is down, the
;; rectangle records on the screen a dot marking its center at each tick.
...
;; dot for recording rectangle on the screen
(define DOT (circle 1 SOLID COLOR-BLACK))
...
(define-struct mark (x y))
;; A Mark is a (make-mark NonNegInt NonNegInt)
;; INTERPRETATION: 
;; x and y are the co-ordinates of the mark on canvas
;; TEMPLATE:
;; mark-fn : Mark -> ??
#|
(define (mark-fn m)
...
;; A ListOfMarks is either
;; -- empty
;; -- (cons Mark ListOfMarks)
...
;; examples of ListOfMarks, for testing
(define MARKS-LIST (list (make-mark 10 20) (make-mark 20 30)))


(define-struct rect (x y vx vy selected? mx my pen-down? marks))
;; A Rectangle is a (make-rect NonNegInt NonNegInt Integer Integer Boolean
;;                             NonNegInt NonNegInt Boolean ListOfMarks)
;; INTERPRETATION: 
...
;; pen-down? describes whether the rectangle record on the screen by a dot or not.
;; marks is the list of marks from where rectangle (center of the rectangle) is
;;     travelled when pen is down.
...
(define RECT-AT_200_100-VEL_-12_20-UNSEL
  (make-rect 200 100 -12 20 false 0 0 false empty))
(define RECT-AT_188_120-VEL_-12_20-UNSEL
  (make-rect 188 120 -12 20 false 0 0 false empty))

(define RECT-AT_200_200-VEL_23_-14-UNSEL
  (make-rect 200 200 23 -14 false 0 0 false empty))
(define RECT-AT_223_186-VEL_23_-14-UNSEL
  (make-rect 223 186 23 -14 false 0 0 false empty))

(define RECT-AT_40_30-VEL_-12_-20-UNSEL
  (make-rect (+ X-MIN-LIMIT 10) (+ Y-MIN-LIMIT 5) -12 -20 false 0 0 false empty))
(define RECT-AT_30_25-VEL_12_20-UNSEL
  (make-rect X-MIN-LIMIT Y-MIN-LIMIT 12 20 false 0 0 false empty))

(define RECT-AT_365_30-VEL_12_-20-UNSEL
  (make-rect (- X-MAX-LIMIT 5) (+ Y-MIN-LIMIT 5) 12 -20 false 0 0 false empty))
(define RECT-AT_370_25-VEL_-12_20-UNSEL
  (make-rect X-MAX-LIMIT Y-MIN-LIMIT -12 20 false 0 0 false empty))

(define RECT-AT_365_265-VEL_12_20-UNSEL
  (make-rect (- X-MAX-LIMIT 5) (- Y-MAX-LIMIT 10) 12 20 false 0 0 false empty))
(define RECT-AT_370_275-VEL_-12_-20-UNSEL
  (make-rect X-MAX-LIMIT Y-MAX-LIMIT -12 -20 false 0 0 false empty))

(define RECT-AT_40_270-VEL_-12_20-UNSEL
  (make-rect (+ X-MIN-LIMIT 10) (- Y-MAX-LIMIT 5) -12 20 false 0 0 false empty))
(define RECT-AT_30_275-VEL_12_-20-UNSEL
  (make-rect X-MIN-LIMIT Y-MAX-LIMIT 12 -20 false 0 0 false empty))

(define RECT-AT_40_100-VEL_-12_20-UNSEL
  (make-rect (+ X-MIN-LIMIT 10) 100 -12 20 false 0 0 false empty))
(define RECT-AT_30_120-VEL_12_20-UNSEL
  (make-rect X-MIN-LIMIT 120 12 20 false 0 0 false empty))

(define RECT-AT_100_35-VEL_-12_-20-UNSEL
  (make-rect 100 (+ Y-MIN-LIMIT 10) -12 -20 false 0 0 false empty))
(define RECT-AT_88_25-VEL_-12_20-UNSEL
  (make-rect 88 Y-MIN-LIMIT -12 20 false 0 0 false empty))

(define RECT-AT_360_100-VEL_12_20-UNSEL
  (make-rect (- X-MAX-LIMIT 10) 100 12 20 false 0 0 false empty))
(define RECT-AT_370_120-VEL_-12_20-UNSEL
  (make-rect X-MAX-LIMIT 120 -12 20 false 0 0 false empty))

(define RECT-AT_100_260-VEL_12_20-UNSEL
  (make-rect 100 (- Y-MAX-LIMIT 15) 12 20 false 0 0 false empty))
(define RECT-AT_112_275-VEL_12_-20-UNSEL
  (make-rect 112 Y-MAX-LIMIT 12 -20 false 0 0 false empty))

(define RECT-AT_200_100-VEL_-12_20-SEL_205_105
  (make-rect 200 100 -12 20 true 205 105 false empty))
(define RECT-AT_200_200-VEL_23_-14-SEL_205_205
  (make-rect 200 200 23 -14 true 205 205 false empty))

(define NEW-RECTANGLE-AT-CENTER-OF-CANVAS
  (make-rect HALF-CANVAS-WIDTH HALF-CANVAS-HEIGHT
             INIT-VX INIT-VY false 0 0 false empty))
(define RECT-AT_200_100-VEL_-12_20-UNSEL-WITH-LIST
  (make-rect 200 100 -12 20 false 0 0 true MARKS-LIST))
...
(define (world-after-tick w)
  (if (world-paused? w)
      w
      (make-world (rects-after-tick (world-rects w))
                  (world-paused? w))))
...
|#
;; DESIGN STRATEGY: Use HOF map on rects
(define (rects-after-tick rects)
 (map rect-after-tick rects))
...
(define (rect-unsel-after-tick r)
  (make-rect (rect-x-after-tick (rect-x r) (rect-vx r))
             (rect-y-after-tick (rect-y r) (rect-vy r))
             (rect-vx-after-tick (rect-x r) (rect-vx r))
             (rect-vy-after-tick (rect-y r) (rect-vy r))
             (rect-selected? r)
             (rect-mx r)
             (rect-my r)
             (rect-pen-down? r)
             (rect-marks-after-tick r)))

;; rect-x-after-tick: NonNegInt Integer -> NonNegInt
;; GIVEN: the x co-ordinate of the center of some unselected rectangle
;; and its velocity vx
;; RETURNS: the x co-ordinate of the rectangle after a tick.
;; EXAMPLES:
#|
(rect-x-after-tick 200 -12) = 188
(rect-x-after-tick 40 -12) = 30
(rect-x-after-tick 365 12) = 370
|#
;; DESIGN STRATEGY: Cases on whether Rectangle touch or go past any vertical side
;; of the canvas on the next tick
(define (rect-x-after-tick x vx)
  (cond
    [(rect-touch-or-go-past-left-side? x vx) X-MIN-LIMIT]
    [(rect-touch-or-go-past-right-side? x vx) X-MAX-LIMIT]
    [else (+ x vx)]))

;; rect-y-after-tick: NonNegInt Integer -> NonNegInt
;; GIVEN: the y co-ordinate of the center of some unselected rectangle
;; and its velocity vy
;; RETURNS: the y co-ordinate of the rectangle after a tick.
;; EXAMPLES:
#|
(rect-y-after-tick 100 20) = 120
(rect-y-after-tick 30 -20) = 25
(rect-y-after-tick 265 20) = 275
|#
;; DESIGN STRATEGY: Cases on whether Rectangle touch or go past any horizontal side
;; of the canvas on the next tick
(define (rect-y-after-tick y vy)
  (cond
    [(rect-touch-or-go-past-top-side? y vy) Y-MIN-LIMIT]
    [(rect-touch-or-go-past-bottom-side? y vy) Y-MAX-LIMIT]
    [else (+ y vy)]))


;; rect-vx-after-tick: NonNegInt Integer -> Integer
;; GIVEN: the x co-ordinate of the center of some unselected rectangle
;; and its velocity vx
;; RETURNS: the x velocity of the recatngle after a tick.
;; EXAMPLES:
#|
(rect-vx-after-tick (+ X-MIN-LIMIT 50) -12) = -12
(rect-vx-after-tick (+ X-MIN-LIMIT 5) -12) = 12
(rect-vx-after-tick (- X-MAX-LIMIT 5) 12) = -12
|#
;; DESIGN STRATEGY: Combine simpler functions
(define (rect-vx-after-tick x vx)
  (if (or (rect-touch-or-go-past-left-side? x vx)
          (rect-touch-or-go-past-right-side? x vx))
      (- vx)
      vx))

;; rect-vy-after-tick: NonNegInt Integer -> Integer
;; GIVEN: the y co-ordinate of the center of some unselected rectangle
;; and its velocity vy
;; RETURNS: the y velocity of the recatngle after a tick.
;; EXAMPLES:
#|
(rect-vy-after-tick (+ Y-MIN-LIMIT 50) -20) = -20
(rect-vy-after-tick (+ Y-MIN-LIMIT 10) -20) = 20
(rect-vy-after-tick (- Y-MAX-LIMIT 10) 20) = -20
|#
;; DESIGN STRATEGY: Combine simpler functions
(define (rect-vy-after-tick y vy)
  (if (or (rect-touch-or-go-past-top-side? y vy)
          (rect-touch-or-go-past-bottom-side? y vy))
      (- vy)
      vy))

;; rect-marks-after-tick: Rectangle -> ListOfMarks
;; GIVEN: the unselected rectangle r
;; RETURNS: the list of marks after adding current position of rectangle (center)
;; in the list of marks of given rectangle.
;; EXAMPLES:
#|
(rect-marks-after-tick RECT-AT_200_100-VEL_-12_20-UNSEL-WITH-LIST)
  = (cons (make-mark 200 100) MARKS-LIST)
|#
;; DESIGN STRATEGY: Use template for Rectangle on r
(define (rect-marks-after-tick r)
  (if (rect-pen-down? r)
      (cons (make-mark (rect-x r) (rect-y r)) (rect-marks r))
      (rect-marks r)))
...
(begin-for-test
  ...
  (check-equal?
   (rect-marks-after-tick RECT-AT_200_100-VEL_-12_20-UNSEL-WITH-LIST)
   (cons (make-mark 200 100) MARKS-LIST)
   "Current position of the rectangle should be added in the list.")
...
;; rect-touch-or-go-past-left-side?: NonNegInt Integer -> Boolean
;; rect-touch-or-go-past-right-side?: NonNegInt Integer -> Boolean
;; GIVEN: the x co-ordinate of the center of some unselected rectangle
;; and its velocity vx
;; ANSWERS: wheather the rectangle is touching or going past the
;; specific side after a tick.
;; EXAMPLES:
;; (rect-touch-or-go-past-left-side? 40 -12) = true
;; (rect-touch-or-go-past-left-side? 200 -12) = false
;; (rect-touch-or-go-past-right-side? 360 12) = true
;; (rect-touch-or-go-past-right-side? 200 -12) = false

;; DESIGN STRATEGY: Combine simpler functions
(define (rect-touch-or-go-past-left-side? x vx)
  (<= (+ x vx) X-MIN-LIMIT))

(define (rect-touch-or-go-past-right-side? x vx)
  (>= (+ x vx) X-MAX-LIMIT))


;; rect-touch-or-go-past-top-side?: NonNegInt Integer -> Boolean
;; rect-touch-or-go-past-bottom-side?: NonNegInt Integer -> Boolean
;; GIVEN: the y co-ordinate of the center of some unselected rectangle
;; and its velocity vy
;; ANSWERS: wheather the rectangle is touching or going past the
;; specific side after a tick.
...
;; EXAMPLES:
;; (rect-touch-or-go-past-top-side? 35 -20) = true
;; (rect-touch-or-go-past-top-side? 100 20) = false
;; (rect-touch-or-go-past-bottom-side? 260 20) = true
;; (rect-touch-or-go-past-bottom-side? 100 20) = false

;; DESIGN STRATEGY: Combine simpler functions
(define (rect-touch-or-go-past-top-side? y vy)
  (<= (+ y vy) Y-MIN-LIMIT))

(define (rect-touch-or-go-past-bottom-side? y vy)
  (>= (+ y vy) Y-MAX-LIMIT))
...
;; DESIGN STRATEGY: Use HOF foldr on (world-rects w)
(define (world-to-scene w)
  (foldr place-rectangle EMPTY-CANVAS (world-rects w)))
...
(define (place-rectangle r scene)
  (if (rect-selected? r)
      ...
      (scene-with-rectangle r COLOR-BLUE scene)))
...
(define (place-rectangle-selected r scene)
  (place-image CIRCLE-IMAGE
               (rect-mx r)
               (rect-my r)
               (scene-with-rectangle r COLOR-RED scene)))


;; scene-with-rectangle: Rectangle Color Scene -> Scene
;; GIVEN: a rectangle r, color for rectangle and a scene
;; RETURNS: a scene with rectangle painted on it, an outline rectangle image of
;; given color, RECTANGLE-WIDTH pixels wide and RECTANGLE-HEIGHT pixels high and
;; current velocity of the given rectangle is displayed as a string with font
;; size = FONT-SIZE and font color = color in the center.

;; EXAMPLES:
#|
(scene-with-rectangle RECT-AT_200_100-VEL_-12_20-UNSEL COLOR-BLUE EMPTY-CANVAS)
  = (place-image IMAGE-OF-RECT-AT_200_100-VEL_-12_20-UNSEL 200 100 EMPTY-CANVAS)

(scene-with-rectangle RECT-AT_200_100-VEL_-12_20-SEL_205_105 COLOR-RED EMPTY-CANVAS)
  = (place-image IMAGE-OF-RECT-AT_200_100-VEL_-12_20-SEL 200 100 EMPTY-CANVAS)
|#

;; DESIGN STRATEGY: Use template for Rectangle on r
(define (scene-with-rectangle r color scene)
  (place-image
   ...
   (place-image
    ...
    (rect-x r)
    (rect-y r)
    (place-marks-on-scene (rect-marks r) scene))))


;; place-marks-on-scene: ListOfMarks Scene -> Scene
;; GIVEN: a list of marks of some rectangle on the scene from where it is
;; travelled when pen is down and a scene
;; RETURNS: the scene on which given list of marks displayed as a solid black
;; circle.
;; EXAMPLES:
#|
(place-marks-on-scene MARKS-LIST EMPTY-CANVAS)
  = (place-image DOT 10 20 (place-image DOT 20 30 EMPTY-CANVAS))
|#

;; DESIGN STRATEGY: Use HOF foldr on marks
(define (place-marks-on-scene marks scene)
  (foldr
   ;; Mark Scene -> Scene
   ;; GIVEN: a scene
   ;; RETURNS: a scene after placing DOT centered at mark-x, mark-y position on
   ;; the given scene
   (lambda (mark-arg scene-arg)
     (place-image DOT (mark-x mark-arg) (mark-y mark-arg) scene-arg))
   scene
   marks))
...
(begin-for-test
  ...
  (check-equal?
   (scene-with-rectangle RECT-AT_200_100-VEL_-12_20-UNSEL COLOR-BLUE EMPTY-CANVAS)
   (place-image IMAGE-OF-RECT-AT_200_100-VEL_-12_20-UNSEL 200 100 EMPTY-CANVAS)
   "RECT-AT_200_100-VEL_-12_20-UNSEL should be displayed on scene.")
  
  (check-equal?
   (scene-with-rectangle RECT-AT_200_100-VEL_-12_20-SEL_205_105 COLOR-RED EMPTY-CANVAS)
   (place-image IMAGE-OF-RECT-AT_200_100-VEL_-12_20-SEL 200 100 EMPTY-CANVAS)
   "RECT-AT_200_100-VEL_-12_20-SEL_205_105 should be displayed on scene.")
  
  (check-equal?
   (place-marks-on-scene MARKS-LIST EMPTY-CANVAS)
   (place-image DOT 10 20 (place-image DOT 20 30 EMPTY-CANVAS))
   "List of marks should be displayed on a empty canvas.")
...
;; direction (up, down, left, or right) by 2 pixels/tick. 
;; on "d" key: pen-down? set to true for the selected rectangle
;; on "u" key: pen-down? set to false for the selected rectangle
...
(define (world-after-key-event w kev)
  (cond
    ...
    [(or (key=? kev LEFT-KEY-EVENT)
         ...
         (key=? kev DOWN-KEY-EVENT)
         (key=? kev D-KEY-EVENT)
         (key=? kev U-KEY-EVENT)) (world-after-other-key-events w kev)]
    [else w]))
...
;; GIVEN: the world state w and the arrow/d/u key event kev
...
|#
;; DESIGN STRATEGY: Use HOF map on (world-rects w)
(define (world-after-other-key-events w kev)
  (make-world
   (map
    ;; Rectangle -> Rectangle
    ;; GIVEN: a rectangle rect-arg
    ;; RETURNS: the rectangle that should follow the given rectangle rect-arg after
    ;; the key event kev
    (lambda (rect-arg)
      (rect-after-key-event rect-arg kev))
    (world-rects w))
   (world-paused? w)))
...
;; GIVEN: the rectangle rect and arrow/d/u key event kev
...
;; GIVEN: the selected rectangle rect and arrow/d/u key event kev
...
;; on "d" key: pen-down? of the given rectangle set to true 
;; on "u" key: pen-down? of the given rectangle set to false
;; ignore all other keys.
...
(rect-selected-after-key-event RECT-AT_200_100-VEL_-12_20-SEL_205_105 D-KEY-EVENT)
= (make-rect 200 100 -12 18 true 205 105)

(rect-selected-after-key-event (make-rect 200 100 -12 20 true 205 105 true MARKS-LIST)
                               U-KEY-EVENT)
= (make-rect 200 100 -12 20 true 205 105 false MARKS-LIST)
|#
...
(define (rect-selected-after-key-event rect kev)
  (make-rect (rect-x rect)
             ...
             (rect-mx rect)
             (rect-my rect)
             (rect-pen-after-key-event (rect-pen-down? rect) kev)
             (rect-marks rect)))
...
;; rect-pen-after-key-event: Boolean KeyEvent -> Boolean
;; GIVEN: the pen-down? flag of some rectangle and key event kev
;; RETURNS: true for D-KEY-EVENT and false for U-KEY-EVENT, returns the given flag
;; for other key events.
;; EXAMPLES:
#|
(rect-pen-after-key-event false D-KEY-EVENT) = true
(rect-pen-after-key-event true U-KEY-EVENT) = false
(rect-pen-after-key-event false Q-KEY-EVENT) = false
|#

;; DESIGN STRATEGY: Cases on KeyEvent kev
(define (rect-pen-after-key-event pen-down? kev)
  (cond
    [(key=? kev D-KEY-EVENT) true]
    [(key=? kev U-KEY-EVENT) false]
    [else pen-down?]))
...
(begin-for-test
  ...
  (check-equal?
   (world-after-key-event (make-world
                           ...
                           true) LEFT-KEY-EVENT)
   (make-world (list (make-rect 200 100 -14 20 true 205 105 false empty)) true)
  ...
  (check-equal?
   (rect-after-key-event RECT-AT_200_100-VEL_-12_20-SEL_205_105 LEFT-KEY-EVENT)
   (make-rect 200 100 -14 20 true 205 105 false empty)
   "after left key, a select rectangle should be changed.")

  (check-equal?
   (rect-selected-after-key-event RECT-AT_200_100-VEL_-12_20-SEL_205_105 D-KEY-EVENT)
   (make-rect 200 100 -12 20 true 205 105 true empty)
   "after d key, a select rectangle's pen should be down.")

  (check-equal?
   (rect-selected-after-key-event
    (make-rect 200 100 -12 20 true 205 105 true MARKS-LIST) U-KEY-EVENT)
   (make-rect 200 100 -12 20 true 205 105 false MARKS-LIST)
   "after u key, a select rectangle's pen should be up.")
...
;; velocity (vx, vy) with pen up and empty mark list.
...
(begin-for-test
  (check-equal?
   ...
   (make-rect 200 100 -12 20 false DEFAULT-MX DEFAULT-MY false empty)
...
(define (world-after-mouse-event w mx my mev)
  (make-world
   (map
    ...
    ;; RETURNS: the rectangle that should follow the given rectangle rect-arg after
    ;; the mouse event mev
    (lambda (rect-arg) (rect-after-mouse-event rect-arg mx my mev))
    (world-rects w))
   (world-paused? w)))
  ...
  (check-equal?
   (world-after-mouse-event 
    (make-world (list RECT-AT_200_100-VEL_-12_20-SEL_205_105
                ...
                false)
    230 130 DRAG-EVENT)
   (make-world (list (make-rect 225 125 -12 20 true 230 130 false empty)
                     RECT-AT_200_200-VEL_23_-14-UNSEL)
               false)
