...
;; examples of ListOfSlip, for testing
...
(define (slips-of-specific-professor los color)
  (filter
   ;; Slip -> Boolean
   ;; GIVEN: a slip 
   ;; RETURNS: whether the color of its argument slip is same as the color or not.
   (lambda (slip)
...
(define (slips-without-duplication los)
  (foldr
   ...
   ;; GIVEN: a slip and a list of slips of any one professor
   ;; RETURNS: a list of slips after adding given slip if it is already not present
   ;;     in the given list
   (lambda (slip los-arg)
     (if (student-already-in-list? slip los-arg)
         los-arg
         (cons slip los-arg)))
   empty
   los))
...
;; RETURNS: whether the student represented by slip s is present in the list los or not
;; EXAMPLES:
...
(define (student-already-in-list? slip los)
  (ormap
   ;; Slip -> Boolean
   ;; GIVEN: a slip slip-arg
   ;; RETURNS: whether its argument slip and slip represent the same student or not.
   (lambda (slip-arg)
     (same-student? slip slip-arg))
   los))
...
(define (same-student? s1 s2)
  (or
   (same-name? (slip-name1 s1) (slip-name2 s1) (slip-name1 s2) (slip-name2 s2))
   (same-name? (slip-name1 s1) (slip-name2 s1) (slip-name2 s2) (slip-name1 s2))))
